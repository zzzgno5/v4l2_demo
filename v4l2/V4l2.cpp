...
int v4l2_enum_fmt(int fd, unsigned int fmt, enum v4l2_buf_type type)
{
    ...
    {
        printf("fmt:%s\n", fmtdesc.description);
		printf("fmt_num:%d  this:%d\n", fmtdesc.pixelformat, fmt);
...
int usb_camera_open(const char *dev, v4l2_buf** mV4l2Buf, int w, int h)
{
	...
	ret = v4l2_s_input(cameraFd, 0);
	if(ret < 0)
	{
		printf("v4l2_s_input failed\r\n");
		return -1;
	}

    // V4L2_PIX_FMT_YUYV
    // V4L2_PIX_FMT_MJPEG
	ret = v4l2_enum_fmt(cameraFd,  V4L2_PIX_FMT_MJPEG, V4L2_BUF_TYPE_VIDEO_CAPTURE);
    if(ret < 0)
    {
		printf("v4l2_enum_fmt failed\r\n");
		return -1;
	}
    ...
    // V4L2_PIX_FMT_YUYV
    // V4L2_PIX_FMT_MJPEG
    ret = v4l2_s_fmt(cameraFd, &mWidth, &mHeight,  V4L2_PIX_FMT_MJPEG, V4L2_BUF_TYPE_VIDEO_CAPTURE);
    if(ret < 0)
    {
		printf("v4l2_s_fmt failed\r\n");
		return -1;
	}
    
    *mV4l2Buf = v4l2_reqbufs(cameraFd, V4L2_BUF_TYPE_VIDEO_CAPTURE, 3);
    if(!*mV4l2Buf)
    {
		printf("v4l2_reqbufs failed\r\n");
		return -1;
	}
    
    ret = v4l2_querybuf(cameraFd, *mV4l2Buf);
    if(ret < 0)
    {
		printf("v4l2_querybuf failed\r\n");
		return -1;
	}
    
    ret = v4l2_mmap(cameraFd, *mV4l2Buf);
    if(ret < 0)
    {
		printf("v4l2_querybuf failed\r\n");
		return -1;
    ...
    ret = v4l2_qbuf_all(cameraFd, *mV4l2Buf);
    if(ret < 0)
    {
		printf("v4l2_qbuf_all failed\r\n");
		return -1;
	}

    ret = v4l2_streamon(cameraFd);
    if(ret < 0)
    {
		printf("v4l2_streamon failed\r\n");
		return -1;
	}
    
    ret = v4l2_poll(cameraFd);
    if(ret < 0)
    {
		printf("v4l2_poll failed\r\n");
		return -1;
	}
